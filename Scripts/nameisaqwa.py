    ...
    for root, dirs, files in os.walk(folder_path):
        for file_name in files:
            if 'AQWA' in file_name:
                ...
                if(name_index_int > 3):
                    files_to_rename.append((os.path.join(root, file_name), os.path.join(root, 'cable' + str(name_index_int-3).zfill(2) + extension)))
...
folder_path = os.path.join(folder_path, '4根横缆情况','尾部两根，首部两根')
x = 300  # 起始行
y = 800  # 结束行
