...
def rename_folder(folder_path):
    ...
    for old_dir_name in folders_to_rename:
        new_dir_name = old_dir_name[:-1] + old_dir_name[-1].zfill(2)  # 构造新的文件夹名字
...
def rename_files(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for file_name in files:
            if 'AQWA' in file_name:
                dot_index = file_name.rfind('.')
                name_index_str = file_name[4:dot_index]
                extension = file_name[dot_index:]
                name_index_int = int(name_index_str)
                if(name_index_int < 17):
                    files_to_rename.append((os.path.join(root, file_name), os.path.join(root, 't_' + 'cable' + str(name_index_int).zfill(2) + extension)))
                else:
                    files_to_rename.append((os.path.join(root, file_name), os.path.join(root, 'y_' + 'cable' + str(name_index_int-16).zfill(2) + extension)))
...
def process_csv_files(folder_path, x, y):
    ...
    for item_file_path in files_path:
        ...
        selected_rows['value'] = selected_rows['value'].abs()
        ...
        # 检查是否存在大于50000的数值
        has_gt_50t = (selected_rows['value'] > 500000).any()
        has_gt_50t_df = pd.DataFrame(data=[['has_gt_50t',has_gt_50t]],
                                    columns = [custom_id,'value'])
        
        count_50t = np.sum(selected_rows['value'] > 500000)
        has_gt_50t_count_df = pd.DataFrame(data=[['has_gt_50t_count',count_50t/(y+1-x)]],
                                    columns = [custom_id,'value'])
        ...
        merged_df = pd.concat([selected_rows, pd.DataFrame({'Max Value': max_value, 'Avg Value': avg_value}).T, has_gt_50t_df, has_gt_50t_count_df, headline_df], axis=0)
...
folder_path = os.path.join(folder_path, '20230922')
x = 300  # 起始行
y = 1800  # 结束行
